...
const SERVICE_NAME = 'ms-payment'; // Nom du service
const HOST = 'localhost'; // Hôte de votre service
const PORT = 3001; // Port de ms-payment (vous avez mentionné 3001)

// Fonction pour s’enregistrer auprès d’Eureka
async function registerWithEureka() {
  const instanceId = `${HOST}:${SERVICE_NAME}:${PORT}`; // Identifiant unique: localhost:ms-payment:3001
  const instance = {
    instance: {
  ...
  try {
    ...
    console.log('ms-payment enregistré auprès d’Eureka');
  } catch (error) {
...
app.get('/', (req, res) => {
  res.send('ms-payment fonctionne !');
});
...
app.listen(PORT, async () => {
  console.log(`ms-payment sur port ${PORT}`);
